...
| `MeterReadingResourceTest` | policy gating, usage and total derived from stored figures, **a later reading at a new rate does not change an earlier bill** and editing one does not pick up a newer rate, the rate field is on screen and required, the first reading opens with an empty rate, a grouped rate is stored as a whole integer, a wrong rate is corrected on the field itself, the form prefills the rate and both ends of the previous reading and takes them from the latest *period* rather than the last row written, a meter with no history opens at zero and at now, a closing figure below the opening one is refused, a closing moment before the opening one is refused while an equal one is accepted, author stamped, the create button is available on an empty log, a photo belongs to the end it was uploaded against, photos stay private and unsigned reads are refused, the view screen prints the rate beside the total, nothing outside the allowlist is logged, photo / cascade / bulk delete auditing |
