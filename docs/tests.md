...
| `PanelCacheTest` | the balance badge answers a second request without a query, a save and a delete each invalidate it, a write reaches the overview totals as well as the badge, an unset tariff is cached rather than re-queried, a saved tariff invalidates the rate badge, and **the model rate is never served from the badge cache** |
